...
export class FavoritesService {
  ...
  addArtist(artistId: string) {
    ...
    if (!artist) throw new UnprocessableEntityException(`Artist with ID ${artistId} not found`);

    if (!this.favorites.artists.some(favArtist => favArtist.id === artist.id)) {
  ...
  addAlbum(albumId: string) {
    ...
    if (!album) throw new UnprocessableEntityException(`Album with ID ${albumId} not found`);
  ...
  addTrack(trackId: string) {
    ...
    if (!track) throw new UnprocessableEntityException(`Track with ID ${trackId} not found`);
  ...
  removeArtist(artistId: string) {
    const index = this.favorites.artists.findIndex(favArtist => favArtist.id === artistId);
    if (index === -1) throw new NotFoundException(`Artist with ID ${artistId} not found in favorites`);
    this.favorites.artists.splice(index, 1);
  }

  // Remove album by ID from favorites
  removeAlbum(albumId: string) {
    const index = this.favorites.albums.findIndex(favAlbum => favAlbum.id === albumId);
    if (index === -1) throw new NotFoundException(`Album with ID ${albumId} not found in favorites`);
    this.favorites.albums.splice(index, 1);
  }

  // Remove track by ID from favorites
  removeTrack(trackId: string) {
    const index = this.favorites.tracks.findIndex(favTrack => favTrack.id === trackId);
    if (index === -1) throw new NotFoundException(`Track with ID ${trackId} not found in favorites`);
